...
const WhoPanel = (props) => {
    ...
    function applyFilters(data) {
        ...
        if (sortingMethod === "level") {
            ...
            if (sortingDirection === "ascending") {
                ...
                data.sort((a, b) => b.guild_name.localeCompare(a.guild_name));
            }
        ...
        if (sortingMethod === "groupid") {
            data = data.filter(
                (character) =>
                    character.group_id !== null && character.group_id !== "0"
            );
            ...
            data = data.filter((character) => {
                return (
                    data.filter(
                        (innerCharacter) =>
                            innerCharacter.group_id === character.group_id
                    ).length > 1
                );
            });
        }
        ...
        const firstPassData = data.filter((character) => {
            ...
            splt.forEach((f) => {
                ...
                if (exactMatch) {
                    ...
                    if (character.guild_name === trimmedFilter) {
                        localmatch = true;
                    }
                } else {
                    if (
    ...
    function isStarred(character) {
        ...
        pinnedCharacters.forEach((p) => {
            if (p.name === character.name && p.server === currentServer)
                val = true;
        });
        return val;
    }
                    ...
                    Fetch(
                        ...
                        .then((val) => {
                            ...
                            if (verifyCharacterData(val)) {
                                ...
                                let characters = [];
                                Object.entries(val.characters).forEach(
                                    ([_, character]) => {
    ...
    React.useEffect(() => {
        if (characterData.data === undefined || characterData.data === null)
            return;
        setCharacterCount(characterData.data.character_count);
    }, [characterData]);

    React.useEffect(() => {
        let pinnedCharacters = JSON.parse(
            localStorage.getItem("pinned-players")
        );
        if (pinnedCharacters) {
    ...
    React.useEffect(() => {
        localStorage.setItem(
            "pinned-players",
            JSON.stringify(pinnedCharacters)
        );
    }, [pinnedCharacters]);
        ...
        >
                        ...
                        >
                            ...
                            >
                                <p style={{ fontSize: "1.3rem" }}>
                                    The Group View feature organizes characters
                                    by the group they're in. Groups are
                                    distinguished by different colors.
                                    Characters who are not in a group (or that
                                    are in a group with only hirelings or
                                    offline characters) are not shown. Searching
                                    by name, guild, or location will show any
                                    character that meets those criteria as well
                                    as all characters in their group.
                                </p>
                                <p style={{ fontSize: "1.3rem" }}>
                                    ...
                                    " will show characters who are from the
                                    guild "Best Guild Ever" or who are currently
                                    in "The Marketplace"
                                </p>
                                <a
                                    className="faux-link"
            ...
            {serverStatus !== false || ignoreServerStatus ? (
                paginatedCharacterData && filteredCharacterData ? (
                    ...
                    >
                        {failedAttemptCount
                            ? `Loading character data. Attempt ${
