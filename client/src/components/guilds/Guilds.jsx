...
const Guilds = (props) => {
  ...
  React.useEffect(() => {
    ...
    setGuildsSorted([
      ...guilds
        .filter((guild) =>
          guild.Name.toLowerCase().includes(guildNameSearch.toLowerCase())
        )
        .sort((a, b) => {
          ...
          >
            {playersPaginated ? (
              ...
              <input
                ...
                onChange={(e) => setGuildNameSearch(e.target.value)}
              />
            )}
            <table className={"guild-table" + (isRunning ? " disabled" : "")}>
              <thead className="guild-table-head">
                {playersPaginated ? (
                  <tr>
                    ...
                    >
                      Name <SortSVG className="nav-icon should-invert" />
                    </th>
                    <th
                      className="guild-table-header"
                      onClick={() => handlePlayerSort("level")}
                    >
                      Level <SortSVG className="nav-icon should-invert" />
                    </th>
                    <th
                      className="guild-table-header"
                      onClick={() => handlePlayerSort("location")}
                    >
                      Location <SortSVG className="nav-icon should-invert" />
                    </th>
                    <th
                      className="guild-table-header"
                      onClick={() => handlePlayerSort("laston")}
                    >
                      Last On <SortSVG className="nav-icon should-invert" />
                    </th>
                  </tr>
                ) : (
                  <tr>
                    <th
                      className="guild-table-header"
                      onClick={() => handleGuildSort("name")}
                    >
                      Guild <SortSVG className="nav-icon should-invert" />
                    </th>
                    <th
                      className="guild-table-header"
                      onClick={() => handleGuildSort("server")}
                    >
                      Server <SortSVG className="nav-icon should-invert" />
                    </th>
                    <th
                      className="guild-table-header"
                      onClick={() => handleGuildSort("membercount")}
                    >
                      Member Count{" "}
              ...
              <tbody>
                {playersPaginated
                  ? playersPaginated.map((player, i) => (
                      ...
                      >
                        <td className="guild-table-cell">{player.Name}</td>
                        <td className="guild-table-cell">
                          <span>{player.TotalLevel} </span>
                          <span
                            style={{
                              color: "var(--text-faded)",
                            }}
                          >
                            {playerClassString(player)}
                          </span>
                        </td>
                        <td className="guild-table-cell">{player.Location}</td>
                        <td className="guild-table-cell">
                  ...
                  : guildsPaginated.map((guild, i) => (
                      <tr
                        key={i}
                        className="guild-table-row"
                        onClick={() => auditGuild(guild.Name, guild.Server)}
                      >
                        <td className="guild-table-cell">{guild.Name}</td>
                        <td className="guild-table-cell">{guild.Server}</td>
                        <td className="guild-table-cell">
                          {guild.MemberCount +
            ...
            >
              {playersSorted
                ? playersSorted &&
                  [...Array(Math.ceil(playersSorted.length / PAGE_SIZE))].map(
                    (o, i) =>
                      Math.ceil(playersSorted.length / PAGE_SIZE) > 20 ? (
                        (i === 0 ||
                          i ===
                            Math.ceil(playersSorted.length / PAGE_SIZE) - 1 ||
                ...
                : guildsSorted &&
                  [...Array(Math.ceil(guildsSorted.length / PAGE_SIZE))].map(
                    (o, i) =>
                      Math.ceil(guildsSorted.length / PAGE_SIZE) > 20 ? (
                        (i === 0 ||
                          i ===
                            Math.ceil(guildsSorted.length / PAGE_SIZE) - 1 ||
