...
const Poll = (props) => {
  ...
  const [voteStates, setVoteStates] = React.useState([-1, -1, -1, -1, -1, -1]);
  ...
  React.useEffect(() => {
    ...
    if (ls !== undefined && ls !== null) {
      let mayvote =
        ...
        new Date().getTime() - 1000 * 60 * 60 * 24 * 62;
      setMayVote(mayvote);
    } else {
      setMayVote(true);
    }
  ...
  return mayVote || isPollClosed ? (
    <ContentCluster title="Poll">
      {isPollClosed ? (
        ...
        >
          Got 15 seconds for a poll? All fields are optional. Your feedback
          helps a lot!
        </p>
      )}
      {!isPollClosed && (
