...
const runAnnualReport = (population, reportType) => {
    ...
    population.forEach(({ datetime, argonnessen_playercount, cannith_playercount, ghallanda_playercount, khyber_playercount, orien_playercount, sarlona_playercount, thelanis_playercount, wayfinder_playercount, hardcore_playercount, argonnessen_lfmcount, cannith_lfmcount, ghallanda_lfmcount, khyber_lfmcount, orien_lfmcount, sarlona_lfmcount, thelanis_lfmcount, wayfinder_lfmcount, hardcore_lfmcount, }) => {
        if (new Date().getTime() - datetime.getTime() <=
            ...
            if (dayofweek === 0 &&
                ...
                if (lastSunday === -1) {
                    ...
                    Argonnessen.data.push({
                        x: lastSunday,
                        y: Math.round((totalsThisWeek[0] / entriesThisWeek[0]) * 100) /
                            100 || 0,
                    });
                    Cannith.data.push({
                        x: lastSunday,
                        y: Math.round((totalsThisWeek[1] / entriesThisWeek[1]) * 100) /
                            100 || 0,
                    });
                    Ghallanda.data.push({
                        x: lastSunday,
                        y: Math.round((totalsThisWeek[2] / entriesThisWeek[2]) * 100) /
                            100 || 0,
                    });
                    Khyber.data.push({
                        x: lastSunday,
                        y: Math.round((totalsThisWeek[3] / entriesThisWeek[3]) * 100) /
                            100 || 0,
                    });
                    Orien.data.push({
                        x: lastSunday,
                        y: Math.round((totalsThisWeek[4] / entriesThisWeek[4]) * 100) /
                            100 || 0,
                    });
                    Sarlona.data.push({
                        x: lastSunday,
                        y: Math.round((totalsThisWeek[5] / entriesThisWeek[5]) * 100) /
                            100 || 0,
                    });
                    Thelanis.data.push({
                        x: lastSunday,
                        y: Math.round((totalsThisWeek[6] / entriesThisWeek[6]) * 100) /
                            100 || 0,
                    });
                    Wayfinder.data.push({
                        x: lastSunday,
                        y: Math.round((totalsThisWeek[7] / entriesThisWeek[7]) * 100) /
                            100 || 0,
                    });
                    Hardcore.data.push({
                        x: lastSunday,
                        y: Math.round((totalsThisWeek[8] / entriesThisWeek[8]) * 100) /
                            100 || 0,
                    });
                    Total.data.push({
                        x: lastSunday,
                        y: Math.round((totalsThisWeek[9] / entriesThisWeek[9]) * 100) /
                            100 || 0,
                    });
                    Permanent.data.push({
                        x: lastSunday,
                        y: Math.round((totalsThisWeek[10] / entriesThisWeek[10]) * 100) /
                            100 || 0,
                    });
                    Maximum.data.push({
                        x: lastSunday,
            ...
            if (reportType === "players") {
                ...
                let totalnow = argonnessen_playercount +
                    cannith_playercount +
                ...
                let totalnow = argonnessen_lfmcount +
                    cannith_lfmcount +
                    ghallanda_lfmcount +
                    khyber_lfmcount +
                    orien_lfmcount +
    ...
    output.reverse();
    fs_1.default.writeFile(`../api_v1/population/year${reportType === "players" ? "" : "_groups"}.json`, JSON.stringify(output), (err) => {
        if (err)
            throw err;
    });
    var t1 = new Date();
...
exports.default = runAnnualReport;
