...
const runDayReport = (population, reporttype) => {
    var t0 = new Date();
    ...
    population.forEach(({ datetime, argonnessen_playercount, cannith_playercount, ghallanda_playercount, khyber_playercount, orien_playercount, sarlona_playercount, thelanis_playercount, wayfinder_playercount, hardcore_playercount, argonnessen_lfmcount, cannith_lfmcount, ghallanda_lfmcount, khyber_lfmcount, orien_lfmcount, sarlona_lfmcount, thelanis_lfmcount, wayfinder_lfmcount, hardcore_lfmcount, }) => {
        if (new Date().getTime() - datetime.getTime() <= 1000 * 60 * 60 * 24) {
            ...
            let totalpopulation = argonnessen_playercount +
                cannith_playercount +
            ...
            let totallfms = argonnessen_lfmcount +
                cannith_lfmcount +
                ghallanda_lfmcount +
                khyber_lfmcount +
                orien_lfmcount +
    ...
    nivoData.reverse();
    fs_1.default.writeFile(`../api_v1/population/day${reporttype === "population" ? "" : "_groups"}.json`, JSON.stringify(nivoData), (err) => {
        if (err)
            throw err;
    });
    if (reporttype === "population") {
        fs_1.default.writeFile(`../api_v1/population/latest.json`, JSON.stringify({
