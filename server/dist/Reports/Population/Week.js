...
const runWeekReport = (population) => {
    ...
    population.forEach(({ datetime, argonnessen_playercount, cannith_playercount, ghallanda_playercount, khyber_playercount, orien_playercount, sarlona_playercount, thelanis_playercount, wayfinder_playercount, hardcore_playercount, }) => {
            ...
            if (lastHour === -1) {
                ...
                if (thisHour !== lastHour) {
                    ...
                    Cannith.data[Cannith.data.length - 1].y =
                        Math.round((Cannith.data[Cannith.data.length - 1].y / entriesThisHour[1]) *
                            100) / 100;
                    ...
                    Khyber.data[Khyber.data.length - 1].y =
                        Math.round((Khyber.data[Khyber.data.length - 1].y / entriesThisHour[3]) *
                            100) / 100;
                    Orien.data[Orien.data.length - 1].y =
                        Math.round((Orien.data[Orien.data.length - 1].y / entriesThisHour[4]) * 100) / 100;
                    Sarlona.data[Sarlona.data.length - 1].y =
                        Math.round((Sarlona.data[Sarlona.data.length - 1].y / entriesThisHour[5]) *
                            100) / 100;
                    ...
                    Total.data[Total.data.length - 1].y =
                        Math.round((Total.data[Total.data.length - 1].y / entriesThisHour[9]) * 100) / 100;
                    Argonnessen.data.push({
                        x: datetime,
                        y: 0,
                    });
    ...
    Argonnessen.data[Argonnessen.data.length - 1].y =
        Math.round((Argonnessen.data[Argonnessen.data.length - 1].y / entriesThisHour[0]) *
            100) / 100;
    Cannith.data[Cannith.data.length - 1].y =
        Math.round((Cannith.data[Cannith.data.length - 1].y / entriesThisHour[1]) * 100) / 100;
    Ghallanda.data[Ghallanda.data.length - 1].y =
        Math.round((Ghallanda.data[Ghallanda.data.length - 1].y / entriesThisHour[2]) * 100) / 100;
    Khyber.data[Khyber.data.length - 1].y =
        Math.round((Khyber.data[Khyber.data.length - 1].y / entriesThisHour[3]) * 100) / 100;
    Orien.data[Orien.data.length - 1].y =
        Math.round((Orien.data[Orien.data.length - 1].y / entriesThisHour[4]) * 100) / 100;
    Sarlona.data[Sarlona.data.length - 1].y =
        Math.round((Sarlona.data[Sarlona.data.length - 1].y / entriesThisHour[5]) * 100) / 100;
    Thelanis.data[Thelanis.data.length - 1].y =
        Math.round((Thelanis.data[Thelanis.data.length - 1].y / entriesThisHour[6]) * 100) / 100;
    Wayfinder.data[Wayfinder.data.length - 1].y =
        Math.round((Wayfinder.data[Wayfinder.data.length - 1].y / entriesThisHour[7]) * 100) / 100;
    Hardcore.data[Hardcore.data.length - 1].y =
        Math.round((Hardcore.data[Hardcore.data.length - 1].y / entriesThisHour[8]) * 100) / 100;
    Total.data[Total.data.length - 1].y =
        Math.round((Total.data[Total.data.length - 1].y / entriesThisHour[9]) * 100) / 100;
    let nivoData = [
        Argonnessen,
    ...
    nivoData.reverse();
    fs_1.default.writeFile("../api_v1/population/week.json", JSON.stringify(nivoData), (err) => {
        if (err)
            throw err;
    });
    var t1 = new Date();
