...
var t0 = new Date();
console.log("Running: 'PopulationReportQuarter'");
...
con.connect(function (err) {
	...
	let q =
		...
		GetDateString(
			new Date(
				new Date(new Date().toDateString()) - 60000 * 60 * 24 * (7 * 13 + 1)
			)
		) +
		"' AND '" +
	...
	con.query(q, function (err, result, fields) {
		...
		result.forEach(
			({
				...
				// This shit takes the current date, removes the time component (so goes to the very start of today),
				//
				datetime = new Date(
		...
		Ghallanda.data[Ghallanda.data.length - 1].y =
			Math.round(
				(Ghallanda.data[Ghallanda.data.length - 1].y / entriesThisDay[2]) *
					100
			) / 100;
		...
		Thelanis.data[Thelanis.data.length - 1].y =
			Math.round(
				(Thelanis.data[Thelanis.data.length - 1].y / entriesThisDay[6]) *
					100
			) / 100;

		Wayfinder.data[Wayfinder.data.length - 1].y =
			Math.round(
				(Wayfinder.data[Wayfinder.data.length - 1].y / entriesThisDay[7]) *
					100
			) / 100;

		Hardcore.data[Hardcore.data.length - 1].y =
			Math.round(
				(Hardcore.data[Hardcore.data.length - 1].y / entriesThisDay[8]) *
					100
			) / 100;
		...
		fs.writeFile(
			"api_v1/data/composite/90_days_by_day.json",
